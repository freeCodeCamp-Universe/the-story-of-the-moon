...
test('should hide keyboard-shortcut controls below 900px', async ({ page }) => {
  ...
  await expect(
    page.getByRole('switch', { name: /enable global keyboard shortcuts/i })
  ).toBeHidden();
  await expect(
    page.getByRole('switch', { name: /enable animations/i })
  ).toBeVisible();
});

test('should show keyboard-shortcut controls at 900px and up', async ({
  page,
}) => {
