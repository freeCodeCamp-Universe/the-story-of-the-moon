...
describe('Chapter', () => {
  it('should expose a labeled region using the chapter heading', () => {
    render(
      <Chapter id="chapter-3" question="How did the Moon form?" title="A violent beginning">
        <p>Body copy</p>
      </Chapter>
    );
    ...
    expect(within(chapter).getByText('How did the Moon form?').textContent).toBe('How did the Moon form?');
  });

  it('should render body content after the chapter header', () => {
    render(
      <Chapter id="chapter-5" question="What did the samples show?" title="Reading the rocks">
    ...
    expect(body?.contains(screen.getByText('The oldest highlands are rich in anorthosite.'))).toBe(true);
    expect(body?.contains(within(chapter).getByRole('button', { name: 'Compare samples' }))).toBe(true);
  });
});
