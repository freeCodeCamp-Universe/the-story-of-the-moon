...
import { ANIMATIONS_STORAGE_KEY, REDUCED_MOTION_ATTRIBUTE, useAnimationsPreference } from '@/hooks/useAnimationsPreference';
...
function PreferenceHarness() {
  ...
  return (
    <>
      <output aria-label="animations-state">{animationsEnabled ? 'on' : 'off'}</output>
      <button type="button" onClick={() => setAnimationsEnabled((current) => !current)}>
        Toggle animations
      </button>
    </>
  );
